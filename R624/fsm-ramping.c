...
void set_power_path(uint8_t ramp_level){
    ...
    // If it doesn't, no need to change anything. If it does, does it
    // come from the top (brighter) or bottom (dimmer?). If it comes 
    // from the top, it's OK to immediately jump from a lower resistance
    // power path to a higher one. However, it it comes from the bottom,
    // due to the way the PWM circuit is converted to an analog voltage
    // via a RC filter, there is some delay (~time constant x 3) before
    // this reference voltage drops from high to low, so a delay needs
    // to be added. This delay is called infrequently, so it's OK to 
    // delay via a busy for loop. We want to avoid using TCPWMs. 
